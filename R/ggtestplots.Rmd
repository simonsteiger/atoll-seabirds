...
axs <- matrix %>% select(!c(atoll, region, lat, long, land_area_sqkm)) # create atoll x species matrix
axs <- axs %>% filter(rowSums(.) > 1) # drop 'empty' atolls

nmds <- metaMDS(comm = axs,
                distance = "bray",
                engine = "monoMDS",
...
plot(vegdist(axs), dist(scores(nmds, "sites")))

stressplot(object = nmds, lwd = 5)

nmds.point <- data.frame(nmds$points)
nmds.point$atoll <- matrix[which(rowSums(matrix[,-c(1:5)]) > 0),]$atoll
...
sp <- wascores(x = nmds$points, w = axs, expand = TRUE) %>% as.data.frame()
