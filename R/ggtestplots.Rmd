...
map + 
  geom_hdr(data = spatspec, aes(x = coordspec[,1], y = coordspec[,2]),
           xlim = c(0,250),
           method = "kde",
           probs = seq(from = 0.1, to = 0.95, length.out = 8)) +
  #geom_point(data = spatspec, aes(x = coordspec[,1], y = coordspec[,2]), alpha=0.2) +
  #geom_raster(data = spatspec, aes(x = coordspec[,1], y = coordspec[,2], fill = dens)) +
  geom_sf(data = mapWorld, fill = "#EBE3D5", color = NA) +
  geom_sf(data = mapCoast, linewidth = 0.15) +
  coord_sf(xlim = c(46.5, 255),
           ylim = c(-30, 30)) +
