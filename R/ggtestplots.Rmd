...
# standardize by land area [ha]
comb$excretedN <- comb$excretedN/(comb$land_area_sqkm*100)
comb$excretedP <- comb$excretedP/(comb$land_area_sqkm*100)
...
pnut.a <- ggplot(data = atoll[which(atoll$nbirds > 0),], aes(x = "identity", y = totN)) +
            ...
            ylab(expression(paste("imported nitrogen [kg N ", ha^-1, " ", year^-1,"]"))) +
...
pnut.c <- ggplot(data = atoll[which(atoll$nbirds > 0),], aes(x = "identity", y = totP)) +
            ...
            scale_colour_gradientn(colours = c("#fbe725", "#3dbc74", "#306b84", "black"),
                                   values = c(1, 0.001, 0.0001, 0)) +
            ylab(expression(paste("imported phosphorous [kg P ", ha^-1, " ", year^-1,"]"))) +
...
pnut.b <- ggplot(data = nutrientinput[which(nutrientinput$nbirds > 0),],
            ...
            scale_fill_viridis_d(aesthetics = "fill", option = "mako") +
            theme_classic() +
            xlab(expression(paste("imported nitrogen per atoll [kg N ",ha^-1, " ", year^-1,"]"))) +
...
pnut.d <- ggplot(data = nutrientinput[which(nutrientinput$nbirds > 0),],
            ...
            scale_fill_viridis_d(aesthetics = "fill", option = "mako") +
            theme_classic() +
            xlab(expression(paste("imported phosphorous per atoll [kg P ",ha^-1, " ", year^-1,"]"))) +
...
pnut.e <- ggplot(data = comb[which(comb$excretedN > 0.09),],
                 aes(y = reorder(group, excretedN), x = excretedN, fill = group)) +
              geom_boxplot() +
              ...
              scale_y_discrete(
                labels = c("Tropicbirds","Frigatebirds" , "Terns", "Petrels &\n Shearwater", "Boobies", "Albatros")) +
              scale_fill_manual(values = c("#0B0405FF", "#382A54FF", "#395D9CFF", "#60CEACFF", "#DEF5e5FF", "#3497a9FF")) +
              theme_classic() +
              xlab(expression(paste("imported nitrogen per atoll [kg N ",ha^-1, " ", year^-1,"]"))) +
