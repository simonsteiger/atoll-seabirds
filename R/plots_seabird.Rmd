...
if (Sys.info()["sysname"] == "Darwin") {
  dat <- read.csv(here("data/allpopulations.csv"))
...
# create map layer
map <- ggplot(data = mapWorld) + 
  geom_sf(fill = "#EBE3D5") +
  coord_sf(xlim = c(48, 255),
           ylim = c(-30, 30)) +
  theme_classic() +
  geom_segment(aes(x = 37.7, xend = 37.7, y = -30.1, yend = 30.1), linewidth = 0.8) +
...
p1 <- map + 
  geom_point(data = spat,
             ...
             size = 3.3, shape = 21, alpha = 0.7, stroke = 3) +
  scale_colour_gradientn(colours = c("red", "yellow", "green","black","black"),
                         values = c(1.0, 0.6, 0.1, 0.099, 0)) +
  ggtitle("Seabird population sizes")
...
p2 <- map + 
  geom_point(data = spat,
             ...
             size = 3.3, shape = 21, alpha = 0.7, stroke = 3) +
  scale_colour_gradientn(colours = c("red", "yellow", "green","black"),
                         values = c(1.0, 0.5, 0.05, 0)) +
  ggtitle("Seabird species richness")

# dissimilarity
p3 <- map +
  geom_point(data = spat,
             aes(x = coords[,1], y = coords[,2], colour = lcbd),
             size = 3.3, shape = 21, alpha = 0.9, stroke = 3) +
   scale_colour_gradientn(colours = c("red", "yellow", "green","black"),
                         values = c(1, 0.85, 0.001, 0)) +
  ggtitle("Compositional distinctness of atolls")
...
ggplot(data = spec, aes(x = ratios*100, y = reorder(species, ratios), fill = group)) +
  scale_fill_viridis_d(option = "mako") +
  geom_bar(stat = "identity", color = "black", width = 0.8) +
  guides(x = "axis_truncated", y = "axis_truncated") +
  ggtitle("Global relevance of atolls") +
...
ggplot(data = spec, aes(x = totbiomass/1000, y = reorder(species, totbiomass))) +
...
p6 <- ggplot(data = atoll.sum[which(atoll.sum$nbirds > 0), ], aes(x = "identity", y = nbirds)) +
  geom_violin(fill = "#B5D5C5") +
  geom_boxplot(fill = "#B5D5C5", width = 0.1) +
  ...
  annotate("text", label = "B3b (A4iii) threshold)", x = 1.4, y = 18500, size = 5, fontface = "italic") +
  theme_classic() +
  theme(
...
p8 <- ggplot(data = atoll.sum[which(atoll.sum$nbirds > 0), ], aes(x = "identity", y = totbiomass/1000)) +
  geom_violin(fill = "#B5D5C5") +
  geom_boxplot(fill = "#B5D5C5", width = 0.1) +
  geom_jitter(width = 0.15, size = 1.5) +
  scale_y_continuous(trans = "log",
...
p10 <- map +
 ...
 # scale_colour_manual(values = c("#ECEE81", "#F79CA5" ,"#F47683" , "#EF4E5E" ,"#EB2438", "#D10015")) +
