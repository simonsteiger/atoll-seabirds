...
atoll <- comb %>% group_by(atoll, region, lat, long, land_area_sqkm) %>%
...
```{r analysis of missingness}

spat.missing <- missing %>% arrange(rev(seabird_data)) %>% st_as_sf(., coords = c("long", "lat"), crs = 4326)
coords.missing <- spat.missing %>% st_coordinates(extract())

map.missing <- map + 
                geom_point(data = spat.missing,
...
ggsave(plotmissing, filename = "fig_analysis_missing01.svg", dpi = 300, width = 210, height = 250, units = "mm")
ggsave(map.missing, filename = "fig_analysis_missing02.svg", dpi = 300, width = 210, height = 100, units = "mm")
